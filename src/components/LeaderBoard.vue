            ...
            <v-row v-for="(group, index) in organizedGroups" :key="group.id">
                ...
                <div v-if="index === 0" class="first-place-container">
                    <v-row>
                        ...
                        <v-chip color="#00aeff" class="subtitle-2" dark>{{group.points}} Points</v-chip> 
                    </v-row>
                    <v-row>
                        <v-col cols="12" md="2" class="text-center mr-2" style="margin-left: -20px">
                            <v-icon 
                                size="100px" 
                                color="#FFD700"
                                class="mt-2 ml-1"
            ...
            <v-row class="justify-center">
                ...
                >
                    ...
                    <v-card-title>{{group.name}}</v-card-title>
                        <v-card-text>
                            <h2 class="white--text"><v-icon>mdi-medal</v-icon> {{index + 2}}<sup>{{group.suffix}}</sup></h2>
                            <p class="mt-4 white--text subtitle-1">{{group.points}} Points</p>
                        </v-card-text>
                    </v-card>
                </div>
            </v-row>
